 ...
 * Shares entities. Transaction control of the save(), update() and delete()
...
public class SharesDAO extends BaseHibernateDAO {
	...
	public Shares findById(java.lang.Integer id) {
		...
		try {
			Shares instance = (Shares) getSession().get("cn.bst.model.Shares", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(Shares instance) {
		...
		try {
			List results = getSession().createCriteria("cn.bst.model.Shares").add(Example.create(instance)).list();
	...
	public List findByProperty(String propertyName, Object value) {
		log.debug("finding Shares instance with property: " + propertyName + ", value: " + value);
		try {
			String queryString = "from Shares as model where model." + propertyName + "= ?";
	...
	public Shares merge(Shares detachedInstance) {
		...
		try {
			Shares result = (Shares) getSession().merge(detachedInstance);
